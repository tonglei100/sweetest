...
我们再写测试用例时，可以直接使用 <_keywords>, <_title> 这些变量。如测试用例:BAIDU_005
...
当自动化运行时，会自动去查找对应的测试数据文件，如果有，则会顺序读取文件行，当读到某一行 flag 不为‘Y’时，就把改行数据导入到变量列表里，同时把测试数据文件中的改行的 flag 列写入'Y'。
需要注意的是，当测试数据文件中的所有行的 flag 都为'Y'时，就无数据导入了，测试用例执行就会失败。所以需要在测试数据文件中准备好足够的数据行，或者及时维护添加数据。
