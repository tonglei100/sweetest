...
class TestSuite():
	...
	def to_xml(self, doc):
		...
		node.setAttribute("failures_detail", "H:%s M:%s L:%s" %(self.high_failures, self.medium_failures, self.low_failures))
		node.setAttribute("errors", "%s" %self.errors)
		node.setAttribute("errors_detail", "H:%s M:%s L:%s" %(self.high_errors, self.medium_errors, self.low_errors))
